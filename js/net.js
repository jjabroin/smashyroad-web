...
export class NetRoom {
  ...
  async joinRoom(code, myCarId) {
    ...
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('host unreachable')), 15000);
      conn.on('open', () => {
        clearTimeout(timer);
  ...
  _onData(peerId, msg) {
    ...
    if (this.isHost) {
      ...
      if (msg.t === 'lobby') {
        ...
        this._emit({ type: 'lobby', players: this.players, trackId: msg.trackId });
      } else if (msg.t === 'start') {
        this.phase = 'racing';
  ...
  broadcastLobby() {
    ...
    const msg = { t: 'lobby', players: this.players, trackId: this.trackId };
  ...
  startRace(aiCarIds) {
    ...
    this.phase = 'racing';
    const msg = { t: 'start', trackId: this.trackId, players: this.players, ai: aiCarIds };
    for (const [, c] of this.conns) {
      try { c.send(msg); } catch (e) { /* 무시 */ }
    }
    return msg;
